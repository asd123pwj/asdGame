...
所以 $self.parent 的级数是跟着链走的，写在菜单项上的指令按"数级数"定位宿主：
  MiniHUD
   └─ Menu(菜单A)            ← 右键 MiniHUD 打开，挂在 MiniHUD 下
      ├─ Close               → UIInteract.close $self.parent.parent           (2 级 = MiniHUD)
      └─ Edit(菜单项)        → UIInteract.open $self MenuEdit $self   (菜单B 挂在 Edit 下)
         └─ MenuEdit(菜单B)
            ...
            │                            events   : open_ui  $self.parent.parent.parent.parent CloseButton $self.parent.parent.parent.parent
            │                                       + Utils.swap "$self.config.events" "$self.config.events_2"
            │                                       + Utils.swap "$self.config.content" "$self.config.content_2"
            │                                       + $self.refresh("content")
            │                            events_2 : 同样三条，只是第一条换成 close $self.parent.parent.parent.parent CloseButton
            ├─ EnableDrag     → $self.parent.parent.parent.parent              (4 级 = MiniHUD)
            └─ Advanced       → UIInteract.open $self MenuEmpty $self  (菜单C 挂在 Advanced 下)
也就是说：**链每深一层，到宿主就多一级 $self.parent**。
...
var values: Array[Array] = [
    ...
    ["Menu", "UI_Panel", {
        ...
        "children": [
            ...
            ["Close", "UI_Label", {
                "content": "关闭",
                "events": [[QName.mouseLeft, "UIInteract.close $self.parent.parent"]],
            }],
            ...
            # $self.parent.parent = 宿主（本项挂在 Menu 下，Menu 挂在宿主下）
            ["CopyName", "UI_Label", {
                "content": "复制名称",
                "events": [[QName.mouseLeft,
                    "Utils.copy $self.parent.parent.config.reg_name"]],
            }],
            # 绑定：悬停弹出子菜单，里面填"要发给哪个 UI"（输入框预填刚复制的名字，回车提交）
            # 传 $self：子菜单挂在**这一项**下面（和"菜单编辑"一个规矩），所以菜单项里的 $self.parent 级数照旧
            # 打开就是通用的 open（--close_on_move：挪开就收）；"预填 + 聚焦"是紧接着的另一条普通指令
            # （事件串本来就支持 \v 连多条，不必为此造一个特殊的"打开"命令）
            ["Bind", "UI_Label", {
                "content": "绑定 ▸",
                "events": [[QName.pointer_enter,
                    "UIInteract.open $self MenuBind $self --close_on_move"]],
            }],
            # 菜单编辑：悬停即在它右上角弹出子菜单（多级菜单 = 菜单开菜单）
            # --close_on_move：鼠标挪开就自动收起来（写在这儿而不是 MenuEdit 预设里——
            # 关闭行为只跟"在哪开、为什么开"有关，每加一层子菜单都要记得抄一遍预设很容易漏）
            ["Edit", "UI_Label", {
                "content": "菜单编辑 ▸",
                "events": [[QName.pointer_enter,
                    "UIInteract.open $self MenuEdit $self --close_on_move"]],
            }],
        ],
    }],
    # 菜单编辑的子菜单：两项都作用于宿主 UI
    # 关闭行为不写在这儿：开它的那句（Menu 的 Edit 项）传了 --close_on_move
    ["MenuEdit", "UI_Panel", {
        "size": [170, 0],
        "free": true,
        ...
        "children": [
            ...
            ["CloseToggle", "UI_Label", {
                ...
                # 路径用双引号包住 = 字面字符串（`$` 开头也不会被当成取值式），所以不用手写 `\$`。
                # **指令串外层一律用单引号**（Godot 和 Python 一样两种引号都行）：
                # 里面要写双引号（路径/字符串参数）时就不用转义成 `\"`，看起来就是指令本身的样子。
                "events": [
                    [QName.mouseLeft, 'UIInteract.open $self.parent.parent.parent.parent CloseButton $self.parent.parent.parent.parent'
                ...
                "events_2": [
                    [QName.mouseLeft, 'UIInteract.close $self.parent.parent.parent.parent CloseButton'
                        + '\vUtils.swap "$self.config.events" "$self.config.events_2"'
                        + '\vUtils.swap "$self.config.content" "$self.config.content_2"'
                        + '\v$self.refresh("content")'],       # 只刷 content（events 不显示，刷它没意义）
                ],
            ...
            ["EnableDrag", "UI_Label", {
                ...
                "events": [[QName.mouseLeft,
                    'UIInteract.switch_value $self.parent.parent.parent.parent events $QName.UI_event_mouseLeft_drag'
                    + '\vUtils.swap "$self.config.content" "$self.config.content_2"'
            ...
            ["Advanced", "UI_Label", {
                "content": "高级编辑 ▸",
                "events": [[QName.mouseLeft, "UIInteract.open $self MenuEmpty $self --close_on_blur"]],
            }],
        ],
    }],
    ...
    # 往上级数同 MenuEdit 的规矩：菜单挂宿主、本项挂 Menu、MenuBind 挂本项 ⇒ 4 级 = 宿主。
    # 关闭行为不写在这儿：开它的那一项传了 --close_on_move（悬停展开 ⇒ 挪开就收）
    ["MenuBind", "UI_Panel", {
        "size": [220, 0],
        "free": true,
        ...
        "children": [
            ...
            ["Name", "UI_Input", {
                "size": [200, 0],
                "events": [
                    [QName.mouseLeft, 'UIInteract.begin_edit $self'],           # 点它进编辑
                    [QName.input_submit,
                        'Utils.write "$self.parent.parent.parent.parent.config.bind" $self.control.text'
