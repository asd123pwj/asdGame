...
事件名就是状态名（见 Archetype_System 的 statuses，如 "Mouse Left | Hold"）——UI 不关心键位，
...
var values: Array[Array] = [
    ["MiniHUD", "UI_Panel", {
        ...
        # events / events_2 成对：菜单项"启用拖拽"靠对调它俩来加/减"整块面板可拖"（这套不含拖动）。
        "events": [[QName.mouseRight_hold, "UIInteract.open $self Menu $self"]],
        "events_2": [
            [QName.mouseRight_hold, "UIInteract.open $self Menu $self"],
            [QName.mouseLeft_hold, "UIInteract.drag $self $event"],
        ],
        "children": [
            ...
            ["Title", "UI_Label", {
                ...
                "events": [
                    [QName.mouseLeft_hold, "UIInteract.drag $parent $event"],
                ],
            }],
            # 关闭"按钮"：就是文本元素 + "Mouse Left | Hold"(左键按住) 指令（点它关闭挂载对象即父 UI）
            ["Close", "UI_Label", {
                "content": "[关闭]",
                "events": [
                    [QName.mouseLeft_hold, "UIInteract.close $parent"],
                ],
            }],
    ...
    ["CloseButton", "UI_Image", {
        ...
        "events": [["Mouse Left | Hold", "UIInteract.close $parent"]],
    }],
    ...
    ["ResizeButton", "UI_Image", {
        ...
        "events": [[QName.mouseLeft_hold, "UIInteract.rescale $parent $event"]],
    }],
]
