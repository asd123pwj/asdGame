...
| `update()` | 每帧过一遍：不满足就删，满足就调回调（由 Tick 的快捷指令驱动） |
| `autos: Array` | 登记表，`[角色, 状态名, Callable]`（探查/调试用） |

## 谁在驱动它

`Config/Character/Archetype/Archetype_System.gd` 的"指针交互"组里：

```gdscript
["Auto Tick", "Tick", "AutoSys.update"],
```

`update()` 用**副本**遍历，因为指令里可能又登记/注销（例如一个开关换来换去）。
