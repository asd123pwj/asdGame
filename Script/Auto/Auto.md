...
- `event` 由 `指令系统（`@self`/`@host`/`@event`）` 换成**带引号的触发事件名**（也就是状态名 / Key 名），
...
- **不用 `"Right | Tick"` 这类逐帧状态**：① 配置里现在也没有它（`Archetype_System` 里注释着）；② 即使有，那种事件也只发给指针当前 hover 的元素——指针一离开就断。现在的"每帧"统一由本类的 `_process` 驱动（消费者：`UIInteract.rescaling` 等比缩放、`UIInteract.dragging` 按住拖动）。
