...
- 把各域消息统一到一处，避免记多个类。用 ASCII 分隔便于定位。主要分块：
  ...
  - **Character 各域**：经 `_format_character` 生成"某角色的某类型某 action"精确 id，细分 ATTR/ANY_ATTR、BUFF(±consume/depleted，**另有 `ANY_BUFF` 通配**：不关心是哪个 buff、只关心它变了就订它)、STATUS(satisfied/unsatisfied/detected/±add/remove/trigger_changed)、INTERACTION、SKILL、COLLISION、INVENTORY、SHORTCUT 的 ±add/remove/act/enter 等。
