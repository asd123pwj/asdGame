...
- 监听器 `ListenType` = `match_type(比较符) + name(监听对象) + thres(阈值)`。attrs 支持 Changed/>/</Base*/AnyChanged 等；buffs Present/Absent；statuses Satisfied/Unsatisfied；interactions Present/Absent/Act；keys 按 HOLD(按住)/PRESS(按下瞬间)/RELEASE(抬起)；time 时间推进；with_detect 用外部检测信号。
