...
var source_attr_category: String
var compare_attr_category: String
var target_attr_category: String
var isPositive: bool
...
func interact(_source: Character, _target: Character, _config: Variant) -> Enums.Code:
    return Enums.Code.NULL

## 攻击比防御高(isMax=True)，则降低(isPositive=False)生命，降低值为攻击防御差值
func like_attack() -> void:
    isPositive = false; isMax = true; isMyself = false

## 治疗量比生命高(isMax=True)，则增加(isPositive=False)生命，增加值为治疗生命差值
func like_heal() -> void:
    isPositive = true; isMax = true; isMyself = false
...
func impact(
        ...
        char_compare: Character, 
        char_target: Character, 
        ) -> ChangeResult:
    """ 属性交互，
        有影响者A，比较对象B，受影响者C 
        基于当前值进行影响
    ...
    var value_source = attr_source.get_(source_attr_category)
    var value_compare = attr_compare.get_(compare_attr_category)
    var value_target = attr_target.get_(target_attr_category)
