...
##
## **想看它长什么样**：角色属性一览（外壳 = `Config/UI/UIPreset_View.gd` 表里的 `Attr` 一行 + 内容元素 `Script/UI/UI/UI_Attr.gd`，
## 就在"角色数据看板"里的那一格，`UIInteract.open(preset_name="RoleData")`）——本类身上那五样（`attributes` / `attributes_before` /
