...
##       "size": [240, 0],
##       "events": [
##           [QName.pointer1_hold, 'UIInteract.begin_edit(@self)'],        # 点它进编辑（换事件就改这一条）
##           [QName.input_submit,
...
##   · 宽度 = **上限本身**（短内容也不缩）：**输入框是表单控件，同一栏里的框要对齐**——
##     这点与只读文本不同（`UI_Label` 是 `min(自然宽, 上限)`，短内容会缩，见它的 `_content_size`）；
...
## 事件本身走的还是项目自己的链（配置里的 `QName.pointer1_hold` 这类 + 需要时冒泡给父级）。
...
## **宽度：有上限（`max_width` / `max_chars`）就取上限那个宽**——短内容也不缩（输入框按上限对齐，
## 与只读文本的 `min(自然宽, 上限)` 不同，见 UI_Label）；没上限就交给容器。
