...
##
## **对外只有一套指令名 `UIInteract.xxx`**（元素按事件发送，如 `UIInteract.close $parent`），
...
##   Interact/UIInteract_Content.gd     改显示内容    → UIInteract.set_content
##   Interact/UIInteract_SwapConfig.gd  对调配置      → UIInteract.swap_config
##
...
## target 为目标 UI 实例（指令里的 $parent/$self 由 UIBase._resolve_cmd 转成 $@ID，
