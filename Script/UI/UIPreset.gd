...
## 按 ui_name 实例化对应 UIBase 并注册进 static _we；开启由 UiSystem.open_ui 统一负责。

## 预设名（独立 UI 的登记名就是它）。
## 被谁用：UiSystem.open_ui（按名查）、UIPreset.get_。
var name: String
## 实现类名（UIBase 子类名，如 "UI_Panel"）。
## 被谁用：create_element（靠 ProjectSettings 的全局类表按类名找脚本）。
var ui_name: String
## 该 UI 的配置（见 Config/UI/）。
## 被谁用：_get_ui_by_name（造模板）、UiSystem._build_open（独立 UI 用它、寄主型深拷贝它）。
var config: Dictionary
## 本预设的模板实例（一份，代表"这个预设"）。
## 被谁用：UiSystem._build_open（**只有独立 UI** 直接 build 它）。
## 注意它是共享实例：寄主型（菜单等）走 create_element 造新实例 + 深拷贝配置，别往它上面存实例态。
var ui: UIBase

## 全部预设。
## 被谁用：UIPreset.get_（UiSystem.open_ui 按名查）。
