...
- **一览类元素**（把运行期数据铺出来看）：`UI_Status`（角色状态）、`UI_Shortcut`（角色快捷）、
  ...
  - **看板**就是一个普通预设（`Config/UI/UIPreset_View.gd` 的 `RoleData`）：**没有专门的元素类**——
    尺寸**在预设里自己按屏幕算**（`_panel_size()` = `UISys.screen_size() × 0.75`：1920×1080 ⇒ 1440×810，换分辨率自动跟）、
...
- **菜单项里的"开关"**（如 `MenuEdit/CloseToggle`）：普通 `UI_Label` 上写两套配置，`"Pointer 1 Hold"` 一条串里做三件事——`UIInteract.open(宿主, "CloseButton", 宿主)`（另一套里是 `UIInteract.close(宿主, "CloseButton")`）\v `Utils.swap "@self.config.events" "@self.config.events_2"` \v `Utils.swap "@self.config.content" "@self.config.content_2"` \v `@self.refresh("content")`，于是点第一次开关闭按钮、点第二次关它，文字也跟着换。关闭按钮就是 `Config/UI/UIPreset_Basic.gd` 里的普通预设（`open_at = Enums.OpenAt.ANCHOR_TOP_RIGHT_IN`：开在锚点**内部**右上角，按自己宽度内缩）。**窗口自带的图标关闭（`UIPreset_Basic.close_item()`）元素名与预设同名（`CloseButton`）**——开关按"挂载点 + 预设名"寻址（见上面"登记名规则"），名字对不上就会视作没开过、又建一份（实测：叫 `Close` 时看板出现两个 X）；同款开关还有 缩放手柄（`MenuEdit/RescaleToggle` ⇄ `ResizeButton`）与改尺寸手柄（`MenuEdit/SizeGripToggle` ⇄ `SizeGrip`）。
