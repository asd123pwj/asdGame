...
- 静态状态：`mouse_position`、`mouse_delta`(相对上次鼠标事件位移)、`on_edit`、`keys_holding`(当前按住集合)。
- `static _input(event)`：被 `Sys._input` 调；区分 Key/MouseButton/MouseMotion，按键按下/抬起调 `_send_key_status`。
- `static _process(delta)`：对按住中的 key 每帧 `Msg.send_key_hold`。
...
- 执行函数（静态，可被指令系统按 `PointDetect.xxx` 调用）：
  - `pointer_press()`：`update_targets()` 后命中 UI 调 `ui.on_pointer_press()`（锁定 `dragging_ui`）。
  - `pointer_move()`：无拖动则直接返回；否则 `update_targets()` + `ui.on_pointer_move()`（拖动跟随，天然支持连按/长按）。
  - `pointer_release()`：`ui.on_pointer_release()`，清 `dragging_ui`。
...
- 驱动链路：`StatusPreset_Pointer` 定义按键状态（Pointer Press/Hold/Release、Submit）→ `SystemShortcutPreset_Pointer` 声明"状态满足→`PointDetect.pointer_press` 等指令"→ 满足时 `Msg.send_cmd`。输入全部来自 `InputSys` 的 `Msg` 消息，不用引擎 `gui_input`。
