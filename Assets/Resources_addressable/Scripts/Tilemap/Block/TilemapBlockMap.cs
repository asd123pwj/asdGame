...
public class TilemapBlockMap: BaseClass{
    ...
    public string _get_tile(int x, int y) => _get(x, y).tile_ID;
    ...
    public TilemapTile _get(int x, int y) {
        if (x < block.size.x && y < block.size.y && x >= 0 && y >= 0){
            ...
            return block_right.isExist ? block_right.map._get(x - block.size.x, y) : new() { tile_ID = GameConfigs._sysCfg.TMap_notLoaded_tile };
        }
        else if (y >= block.size.y){
            TilemapBlock block_up = block.around.up;
            return block_up.isExist ? block_up.map._get(x, y - block.size.y) : new() { tile_ID = GameConfigs._sysCfg.TMap_notLoaded_tile };
        }
        else if (x < 0){
            TilemapBlock block_left = block.around.left;
            return block_left.isExist ? block_left.map._get(x + block.size.x, y) : new() { tile_ID = GameConfigs._sysCfg.TMap_notLoaded_tile };
        }
        else if (y < 0){
            TilemapBlock block_down = block.around.down;
            return block_down.isExist ? block_down.map._get(x, y - block.size.y) : new() { tile_ID = GameConfigs._sysCfg.TMap_notLoaded_tile };
        }
