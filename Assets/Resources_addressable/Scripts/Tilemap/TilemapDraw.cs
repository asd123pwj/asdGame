...
public class TilemapDraw: BaseClass{
    ...
    public Dictionary<Vector3Int, TilemapRegion4Draw> _get_draw_regions_placeholder(TilemapBlock block){
        ...
        if (block.map[0, 0] != "0") {
            ...
            region = new() {
                tiles = new TileBase[1],
                positions = new Vector3Int[1],
            };
            TileBase tile = get_placeholder_tile(block, new Vector3Int(0, 0));
            region.tiles[0] = tile;
            region.positions[0] = block_origin_pos;
            regions.Add(block_offset, region);
        }
        // upper left quarter
        if (block.map[0, block.size.y - 1] != "0") {
            block_offset = block.offsets + new Vector3Int(-1, 1, 0);
            region = new() {
                tiles = new TileBase[1],
                positions = new Vector3Int[1],
            };
            TileBase tile = get_placeholder_tile(block, new Vector3Int(0, block.size.y - 1));
            region.tiles[0] = tile;
            region.positions[0] = block_origin_pos + new Vector3Int(0, block.size.y - 1, 0);
            regions.Add(block_offset, region);
        }
        // lower right quarter
        if (block.map[block.size.x - 1, 0] != "0") {
            block_offset = block.offsets + new Vector3Int(1, -1, 0);
            region = new() {
                tiles = new TileBase[1],
                positions = new Vector3Int[1],
            };
            TileBase tile = get_placeholder_tile(block, new Vector3Int(block.size.x - 1, 0));
            region.tiles[0] = tile;
            region.positions[0] = block_origin_pos + new Vector3Int(block.size.x - 1, 0, 0);
            regions.Add(block_offset, region);
        }
        // upper right quarter
        if (block.map[block.size.x - 1, block.size.y - 1] != "0") {
            block_offset = block.offsets + new Vector3Int(1, 1, 0);
            region = new() {
                tiles = new TileBase[1],
                positions = new Vector3Int[1],
            };
            TileBase tile = get_placeholder_tile(block, new Vector3Int(block.size.x - 1, block.size.y - 1));
            region.tiles[0] = tile;
            region.positions[0] = block_origin_pos + new Vector3Int(block.size.x - 1, block.size.y - 1, 0);
            regions.Add(block_offset, region);
        }
        // left side
        block_offset = block.offsets + new Vector3Int(-1, 0, 0);
        region = new() {
            tiles = new TileBase[block.size.y],
            positions = new Vector3Int[block.size.y],
        };
        for (int y = 0; y < block.size.y; y++){
            TileBase tile = get_placeholder_tile(block, new Vector3Int(0, y));
            region.tiles[y] = tile;
            region.positions[y] = block_origin_pos + new Vector3Int(0, y, 0);
        }
        regions.Add(block_offset, region);
        // right side
        block_offset = block.offsets + new Vector3Int(1, 0, 0);
        ...
        for (int y = 0; y < block.size.y; y++){
            TileBase tile = get_placeholder_tile(block, new Vector3Int(block.size.x - 1, y));
            region.tiles[y] = tile;
            region.positions[y] = block_origin_pos + new Vector3Int(block.size.x - 1, y, 0);
        }
        regions.Add(block_offset, region);
        // bottom side
        block_offset = block.offsets + new Vector3Int(0, -1, 0);
        region = new() {
            tiles = new TileBase[block.size.x],
            positions = new Vector3Int[block.size.x],
        };
        for (int x = 0; x < block.size.x; x++){
            TileBase tile = get_placeholder_tile(block, new Vector3Int(x, 0));
            region.tiles[x] = tile;
            region.positions[x] = block_origin_pos + new Vector3Int(x, 0, 0);
        }
        regions.Add(block_offset, region);
        // top side
        block_offset = block.offsets + new Vector3Int(0, 1, 0);
        ...
        for (int x = 0; x < block.size.x; x++){
            TileBase tile = get_placeholder_tile(block, new Vector3Int(x, block.size.y - 1));
            region.tiles[x] = tile;
            region.positions[x] = block_origin_pos + new Vector3Int(x, block.size.y - 1, 0);
        }
        regions.Add(block_offset, region);
