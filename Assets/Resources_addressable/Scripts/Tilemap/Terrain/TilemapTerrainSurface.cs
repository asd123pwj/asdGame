...
public class TilemapTerrainSurface: BaseClass{
    ...
    // ---------- Horizontal Edge ---------- //
    public int _get_h(Vector3Int map_pos, float frequency, int max, int origin, string type=""){
        ...
        if ((h_ratio > 0 && type == "-") || (h_ratio < 0 && type == "+")){
            ...
            h = Mathf.FloorToInt(max * h_ratio) + origin;
        return h;
    }


    public TerrainType _check_surface(Vector3Int map_pos, TerrainHier1Info hier1){
        int surface_h = _get_h(map_pos, surface_frequency, surface_max, 0);
        float frequency_2;
        foreach (string[] f in hier1.frequency){
            frequency_2 = float.Parse(f[1]);
            surface_h = _get_h(map_pos, float.Parse(f[1]), int.Parse(f[2]), surface_h, f[0]);
        }
        if (map_pos.y > surface_h) return TerrainType.Overground;
        else return TerrainType.Underground;
    }
