...
public class TilemapConfig: BaseClass{
    ...
    public Vector3Int _mapping_worldPos_to_mapPos(Vector3 world_pos, string layer_type){
        ...
        Vector3Int mapPos = new(Mathf.FloorToInt(world_pos.x - offsets.x), Mathf.FloorToInt(world_pos.y - offsets.y), 0);
        return mapPos;
    }
    ...
    public Vector3Int _mapping_blockOffsets_to_zoneOffsets(Vector3Int block_offsets){
        ...
        Vector3Int block_pos = new(Mathf.FloorToInt(block_offsets.x / zone_size.x), Mathf.FloorToInt(block_offsets.y / zone_size.y), 0);
        return block_pos;
    }
    ...
    public Vector3Int _mapping_mapPos_to_blockOffsets(Vector3Int map_pos){
        ...
        Vector3Int block_pos = new(Mathf.FloorToInt(map_pos.x / block_size.x), Mathf.FloorToInt(map_pos.y / block_size.y), 0);
        return block_pos;
    }
    public Vector3Int _mapping_mapPos_to_inBlockPos(Vector3Int map_pos){
        Vector3Int block_size = _GCfg._sysCfg.TMap_tiles_per_block;
        if (map_pos.x < 0) map_pos.x = Math.Abs(block_size.x - map_pos.x);
        if (map_pos.y < 0) map_pos.y = Math.Abs(block_size.y - map_pos.y);
